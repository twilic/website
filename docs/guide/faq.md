...
The current specification family is v3, and implementations are required to be deterministic and interoperable within the explicitly selected version/profile. The current published SDK line remains v2 unless an SDK explicitly advertises v3 support.
...
The v3 specification is finalized. The eighteen official SDKs (Rust, Go, Python, JavaScript/TypeScript, Java, Scala, Ruby, R, Zig, PHP, Kotlin, Dart, Elixir, Lua, C, C++, C#, and Swift) are conformance-tested against shared v2 binary fixtures generated by the Rust reference implementation until their v3 support lands.
...
The `benchmark` and `twilic` repositories include binary fixtures generated by the Rust reference implementation. Each published SDK runs conformance tests against v2 fixtures. For v3, verify against the v3 reference profile, including `BOUND_STREAM`, `SCHEMA_BATCH`, compact Bound payloads, and negotiated extension handling.

### Can I use Twilic between services written in different languages?

Yes. This is a primary design goal. The current published SDK line targets v2 payloads and is tested for interoperability. For v3 payloads, both services must explicitly agree on the v3 version/profile and any negotiated extensions.
